...
def make_pngs(target_filter='f480m', new_basepath='/orange/adamginsburg/jwst/sgrc/data_reprojected/'):
    ...
    if len(filternames) >= 3:
        for i in range(len(filternames) - 2):
            ...
            save_rgb(rgb_scaled, f'{png_path}/SGRC_RGB_{f1n}-{f2n}-{f3n}.png', avm=AVM)
            ...
            save_rgb(rgb_scaled, f'{png_path}/SGRC_RGB_{f1n}-{f2n}-{f3n}_log.png', avm=AVM)
    ...
    if len(filternames_sub) >= 3:
        for i in range(len(filternames_sub) - 2):
            ...
            f1n, f2n, f3n = ''.join(filter(str.isdigit, f1)), ''.join(filter(str.isdigit, f2)), ''.join(filter(str.isdigit, f3))
            save_rgb(rgb_scaled, f'{png_path}/SGRC_RGB_{f1n}-{f2n}-{f3n}_sub.png', avm=AVM)
            ...
            save_rgb(rgb_scaled, f'{png_path}/SGRC_RGB_{f1n}-{f2n}-{f3n}_sub_log.png', avm=AVM)
    ...
    for filtername in filternames:
        ...
        save_rgb(img_asinh, f'{png_path}/SGRC_{fn}_asinh.png', avm=AVM)
        ...
        save_rgb(img_log, f'{png_path}/SGRC_{fn}_log.png', avm=AVM)
