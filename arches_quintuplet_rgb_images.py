...
def make_pngs(target_filter='f323n', new_basepath='/orange/adamginsburg/jwst/arches_quintuplet/data_reprojected/'):
    ...
    if len(filternames) >= 2:
        ...
        for composite_method in ['average', 'difference']:
            ...
            save_rgb(rgb_scaled, f'{png_path}/ArchesQuintuplet_RGB_{f1n}-{composite_method}-{f2n}.png', avm=AVM)
            ...
            save_rgb(rgb_scaled, f'{png_path}/ArchesQuintuplet_RGB_{f1n}-{composite_method}-{f2n}_log.png', avm=AVM)
    ...
    if len(filternames_sub) >= 2:
        ...
        f1n, f2n = ''.join(filter(str.isdigit, f1)), ''.join(filter(str.isdigit, f2))
        save_rgb(rgb_scaled, f'{png_path}/ArchesQuintuplet_RGB_{f1n}-composite-{f2n}_sub.png', avm=AVM)
        ...
        save_rgb(rgb_scaled, f'{png_path}/ArchesQuintuplet_RGB_{f1n}-composite-{f2n}_sub_log.png', avm=AVM)
    ...
    for filtername in filternames:
        ...
        save_rgb(img_asinh, f'{png_path}/ArchesQuintuplet_{fn}_asinh.png', avm=AVM)
        ...
        save_rgb(img_log, f'{png_path}/ArchesQuintuplet_{fn}_log.png', avm=AVM)
